...
fn print_usage(program: &str) {
    ...
    eprintln!("  {} stats <input_json> <output_csv> [trim_start_kernel]", program);
    ...
    match command.as_str() {
        ...
        "stats" => {
            if args.len() < 4 || args.len() > 5 {
                eprintln!("Error: 'stats' requires 2-3 arguments");
                eprintln!("Usage: {} stats <input_json> <output_csv> [trim_start_kernel]", args[0]);
                std::process::exit(1);
            }
            ...
            profile_stats::analyze_profile_stats(input_file, output_file, trim_start_kernel)?;
        }
